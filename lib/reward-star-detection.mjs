...
function isDarkInk(red, green, blue, alpha) {
  return alpha > 180 && red < 96 && green < 96 && blue < 96;
}

function isMostlyLight(pixels) {
  let lightPixels = 0;
  let sampledPixels = 0;

  for (let offset = 0; offset < pixels.length; offset += 64) {
    const luminance = pixels[offset] * 0.2126 + pixels[offset + 1] * 0.7152 + pixels[offset + 2] * 0.0722;
    lightPixels += luminance >= 160 ? 1 : 0;
    sampledPixels += 1;
  }

  return sampledPixels > 0 && lightPixels / sampledPixels >= 0.6;
}

function findComponents(pixels, width, height, matchesColor) {
  ...
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      if (matchesColor(pixels[offset], pixels[offset + 1], pixels[offset + 2], pixels[offset + 3])) {
        mask[index] = 1;
      }
    }
  }
...
function detectSelectedStarsByColor(pixels, width, height, matchesColor) {
  const { components, mask } = findComponents(pixels, width, height, matchesColor);
...
export function detectSelectedStarsFromPixels(pixels, width, height) {
  const blueStars = detectSelectedStarsByColor(pixels, width, height, isAppStoreBlue);
  if (blueStars === 5 || !isMostlyLight(pixels)) return blueStars;
  return detectSelectedStarsByColor(pixels, width, height, isDarkInk);
}
